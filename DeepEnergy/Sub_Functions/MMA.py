...
from Sub_Functions.utils import Timer, constrain
...
def optimize(X0, optimizationParams, objectiveHandle, consHandle, n1, n2):
    rho = X0
    numConstraints = 1
    m = numConstraints
    # num constraints
    ...
    mma.setMinandMaxBoundsForDesignVariables(1e-6 + np.zeros((n, 1)), np.ones((n, 1)))
    ...
    mma.setScalingParams(
        1.0, np.zeros((m, 1)), 10000 * np.ones((m, 1)), np.zeros((m, 1))
    )
    mma.setMoveLimit(0.2)

    print("Iteration │ Objective │ ΔObjective │ Tolerance │ Training time ")
    print("──────────┼───────────┼────────────┼───────────┼───────────────")

    prev_J = None
    delta_J = float("Infinity")
    iteration = 0
    total_io_time = 0.0
    while (
        ...
        J, dJ, training_time, io_time = objectiveHandle(rho, iteration)
        ...
        print(
            f"{iteration+1:^9} │ {constrain(J, 9)} │ {constrain(delta_J, 10)} │ "
            + f"{constrain(optimizationParams['relTol'], 9)} │ "
            + f"{Timer.prettify_seconds(training_time):^13}"
        )
