...
def optimize( X0 , optimizationParams, \
             objectiveHandle, consHandle , n1 , n2 ):
    rho = X0; 
    numConstraints = 1;
    loop = 0; 
    change = 1.;
    ...
    while( (change > optimizationParams['relTol']) \
        ...
        status = 'Iter {:d}; obj fun {:.4F}; vf {:.3F}'.\
                format(loop, J, np.mean( rho ));
        print(status)
        # print( loop , J , np.mean(rho) )

        # if(loop%100 == 0):
        #     img = np.flipud(rho.reshape(( n1 , n2 )))
